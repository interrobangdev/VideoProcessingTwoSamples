...
import Photos
import QuartzCore
@preconcurrency import VideoProcessingTwo

final class VideoFilterPreviewState: ObservableObject {
...
final class VideoFilterViewModel: NSObject, ObservableObject {
    ...
    let previewState = VideoFilterPreviewState()

    private let sceneQueue = DispatchQueue(label: "com.interrobang.VideoFilter.sceneQueue")
    private var scene: VideoScene?
    private var sourceVideoURL: URL?
    private var sourceDuration: Double = 0.0
    private var sourceFrameRate: Double = 30.0
    private var sourceRotation: Double = 0.0
    ...
    private var lastLoopedTime: Double = 0.0
    ...
    var currentEntry: ShowcaseEntry? {
        filterEntries.first { $0.id == selectedFilterID }
    }
    ...
    func startPreview() {
        ...
        let preferredFPS = Int(min(max(sourceFrameRate.rounded(), 24.0), 60.0))
        if #available(iOS 15.0, *) {
            let minimumFPS = Float(min(preferredFPS, 30))
            let targetFPS = Float(preferredFPS)
            link.preferredFrameRateRange = CAFrameRateRange(minimum: minimumFPS, maximum: targetFPS, preferred: targetFPS)
        } else {
        ...
        if playbackStartHostTime == nil {
            playbackStartHostTime = CACurrentMediaTime()
        }
    }
    ...
    func selectFilter(_ id: String) {
        ...
        applyCurrentSelection(rebuildFilters: true)
    }
    ...
    func exportFilteredVideo() {
        ...
        guard let scene = makeExportScene() else {
            ...
            completion: { [weak self] success in
                DispatchQueue.main.async {
                    ...
                    if success {
                        ...
                        self.exportStatusMessage = "Export finished. Saving to Photos..."
                        self.saveExportToPhotoLibrary(url: outputURL)
                    } else {
    ...
    private func configureVideo(url: URL) {
        ...
        let naturalSize = track?.naturalSize ?? CGSize(width: 1920, height: 1080)
        let preferredTransform = track?.preferredTransform ?? .identity
        let orientedSize = Self.orientedVideoSize(naturalSize: naturalSize, transform: preferredTransform)
        let frameRate = Double(track?.nominalFrameRate ?? 0)

        sourceVideoURL = url
        sourceDuration = max(asset.duration.seconds, 1.0 / 30.0)
        sourceFrameRate = frameRate > 0 ? frameRate : 30.0
        currentRenderSize = orientedSize.width > 0 && orientedSize.height > 0 ? orientedSize : CGSize(width: 1920, height: 1080)
        sourceRotation = Self.rotationAngle(for: preferredTransform)
        ...
        playbackStartHostTime = CACurrentMediaTime()
        lastLoopedTime = 0.0
        previewState.displayCIImage = nil

        rebuildPreviewScene()
    }

    private func rebuildPreviewScene() {
        let values = parameterValues
        let selectionID = selectedFilterID
        ...
        sceneQueue.async { [weak self] in
            guard let self else { return }
            self.scene = scene
            if let entry = self.filterEntries.first(where: { $0.id == selectionID }) {
                self.scene?.group.filters = entry.makeFilters(values, renderSize)
                self.activeSelectionID = selectionID
            } else {
                self.scene?.group.filters = []
                self.activeSelectionID = nil
            }
        }
    }

    private func makeBaseScene() -> VideoScene? {
        guard let sourceVideoURL else { return nil }

        let scene = VideoScene(duration: sourceDuration, frameRate: sourceFrameRate, size: currentRenderSize)
        _ = scene.addAsset(
            atLayerIndex: LayerObjectIndex(groupIndices: [], layerIndex: 0),
            type: .video,
            frame: CGRect(origin: .zero, size: currentRenderSize),
            rotation: sourceRotation,
            assetURL: sourceVideoURL,
            text: ""
        )
        return scene
    }

    private func makeExportScene() -> VideoScene? {
        guard let scene = makeBaseScene(), let entry = currentEntry else {
            return nil
        }
        scene.group.filters = entry.makeFilters(parameterValues, currentRenderSize)
        return scene
    }
    ...
    private func applyCurrentSelection(rebuildFilters: Bool = true) {
        ...
        if rebuildFilters || activeSelectionID != selectionID {
            let filters = entry.makeFilters(values, size)
            activeSelectionID = selectionID
            sceneQueue.async { [weak self] in
                self?.scene?.group.filters = filters
            }
            return
        }
    ...
    @objc private func displayLinkFired(_ link: CADisplayLink) {
        guard videoLoaded, sourceDuration > 0, let start = playbackStartHostTime else { return }

        let elapsed = CACurrentMediaTime() - start
        let loopedTime = elapsed.truncatingRemainder(dividingBy: sourceDuration)

        if loopedTime < lastLoopedTime {
            playbackStartHostTime = CACurrentMediaTime() - loopedTime
            lastLoopedTime = loopedTime
            rebuildPreviewScene()
            return
        }
        ...
        sceneQueue.async { [weak self] in
            guard let self else { return }
            let renderedImage = self.scene?.renderScene(frameTime: loopedTime, compositionTimeOffset: 0.0)
    ...
    private static func orientedVideoSize(naturalSize: CGSize, transform: CGAffineTransform) -> CGSize {
        let rect = CGRect(origin: .zero, size: naturalSize).applying(transform).standardized
        let size = rect.size
        guard size.width > 0 && size.height > 0 else { return naturalSize }
        return CGSize(width: abs(size.width), height: abs(size.height))
    }

    private static func rotationAngle(for transform: CGAffineTransform) -> Double {
        Double(atan2(transform.b, transform.a))
    }

    private func saveExportToPhotoLibrary(url: URL) {
        let currentStatus = PHPhotoLibrary.authorizationStatus(for: .addOnly)
        switch currentStatus {
        case .authorized, .limited:
            performPhotoLibrarySave(url: url)
        case .notDetermined:
            PHPhotoLibrary.requestAuthorization(for: .addOnly) { [weak self] status in
                DispatchQueue.main.async {
                    guard let self else { return }
                    if status == .authorized || status == .limited {
                        self.performPhotoLibrarySave(url: url)
                    } else {
                        self.exportStatusMessage = "Export finished, but Photos access was denied."
                        self.errorMessage = "The video was exported locally but could not be saved to your Camera Roll."
                    }
                }
            }
        default:
            exportStatusMessage = "Export finished, but Photos access is unavailable."
            errorMessage = "The video was exported locally but could not be saved to your Camera Roll."
        }
    }

    private func performPhotoLibrarySave(url: URL) {
        guard FileManager.default.fileExists(atPath: url.path) else {
            exportStatusMessage = nil
            errorMessage = "Export finished, but the output file could not be found for Photos save."
            return
        }
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            DispatchQueue.main.async {
                guard let self else { return }
                if success {
                    self.exportStatusMessage = "Saved to Camera Roll."
                } else {
                    self.exportStatusMessage = "Export finished, but saving to Photos failed."
                    self.errorMessage = error?.localizedDescription ?? "The video exported, but could not be saved to your Camera Roll."
                }
            }
        }
    }
